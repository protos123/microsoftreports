...
first_date = dt.datetime(2018,05,30)
last_date = dt.datetime(2018,06,20)
...
transfers=ltr.get_transfers_file_acc_642552()
...
monthlyreport=qp.createmonthlyreportforcolombiaacc642552(first_date=first_date,last_date=last_date)
...
for i in range(1,len(locators)):
    ...
    totalvalue=df2.sum()
    if totalvalue>1:
        filename = 'MSFTPAYU_CO_U1010ECOP01_' + str(datefilename)+ '_0' +'errorsuma'+('.xlsx')
    else:
        filename = 'MSFTPAYU_CO_U1010ECOP01_' + str(datefilename) + '_0' + ('.xlsx')
    batchname= 'MSFTPAYU_CO_U1010ECOP01_' + str(datefilename)+ '_0'
